...
const ballRuntime = fs.readFileSync(path.join(root, 'src/game/Ball.tsx'), 'utf8')
if (!engine.includes('CoreMatchGameplayEngine')) failures.push('Core orchestrator is missing')
if (!ballRuntime.includes('coreEngine.current.tick')) failures.push('Core engine is not integrated into the live physical ball runtime')
if (!ballRuntime.includes('coreResult.newRestart')) failures.push('Authoritative restart safety handling is not connected')
