...
import type { CameraMode, Difficulty, TeamSide, TimeOfDay, Weather } from '../game/types'
...
export function MatchView() {
  ...
  }, [away.name, home.name, score.away, score.home, score.time, setRecords])
  ...
  return (
    <div className="view-stack match-view">
      <section className="page-title-row">
        <div>
          <span className="section-kicker">Phase 1 · 3D engine foundation</span>
          <h1>Broadcast Match Center</h1>
          <p>Play an official-scale 3D match with Rapier ball physics, procedural athletes, dynamic lighting and four camera systems. Move with WASD or arrows, sprint with Shift, pass with E and shoot with Space.</p>
        </div>
        <div className="title-actions"><span className="prototype-badge"><span /> WEBGL 3D · RAPIER PHYSICS</span></div>
      </section>
      ...
      <section ref={shellRef} className="game-shell game-shell-3d">
        ...
        <div className="camera-status"><Camera size={13} /><span>{cameraMode} camera</span></div>
        ...
        <div className="broadcast-lower-third">
          <span><Gamepad2 size={14} /> WASD MOVE</span>
          <span>SHIFT SPRINT</span>
          <span>E PASS</span>
          <span>SPACE SHOOT</span>
          {cameraMode === 'free' && <span>Q / R VERTICAL</span>}
        </div>
      ...
      <section className="match-toolbar panel match-toolbar-3d">
        <div className="match-primary-actions">
          <button className="primary-button" onClick={() => setRunning((value) => !value)}>
            ...
            {running ? 'Pause match' : score.time > 0 ? 'Resume match' : 'Kick off'}
          </button>
        ...
        <div className="camera-picker" aria-label="Camera mode">
          {([
            ...
            <button key={mode} className={cameraMode === mode ? 'active' : ''} onClick={() => setCameraMode(mode)} title={`${label} camera`}>
        ...
        <div className="match-environment-controls">
          <label>Difficulty
            <select value={difficulty} onChange={(event) => setDifficulty(event.target.value as Difficulty)}>
              <option>Academy</option><option>Professional</option><option>Legendary</option>
            </select>
          </label>
          <label>Light
            <select value={timeOfDay} onChange={(event) => setTimeOfDay(event.target.value as TimeOfDay)}>
              <option value="afternoon">Afternoon</option><option value="golden">Golden hour</option><option value="night">Night</option>
            </select>
          </label>
      ...
      <div className="dashboard-grid three">
        <article className="stat-panel panel"><TimerReset /><div><span>World scale</span><strong>105 × 68 metres</strong><small>Official professional pitch dimensions</small></div></article>
        <article className="stat-panel panel"><Shield /><div><span>Simulation</span><strong>Rigid-body football</strong><small>0.43 kg ball, spin, friction and weather</small></div></article>
        <article className="stat-panel panel"><Trophy /><div><span>Presentation</span><strong>4 live cameras</strong><small>Broadcast, follow, ball and free-look</small></div></article>
      </div>
        ...
        <section className="panel history-panel">
          ...
          {records.map((record) => (
            <div className="history-row" key={record.id}>
              <span>{new Date(record.date).toLocaleDateString()}</span><strong>{record.home}</strong><b>{record.homeScore} — {record.awayScore}</b><strong>{record.away}</strong>
            </div>
          ))}
        </section>
      )}
