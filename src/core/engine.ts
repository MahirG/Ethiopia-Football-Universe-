...
export class CoreMatchGameplayEngine {
  ...
  tick(frame: CoreFrameInput): CoreFrameResult {
    ...
    this.statistics.tick(frame.running ? frame.delta : 0, possession.team, possession.state === 'contested', this.ballInPlay)
    if (frame.running) this.statistics.captureFrame(frame.simulationTime, frame.ball.position, frame.players.map((player) => ({ id: player.id, position: player.position })))
